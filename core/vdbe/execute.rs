...
///
/// This reads CDC data from registers and publishes to the streaming sink if one is configured.
/// If no sink is configured, this is a no-op. Publish errors are logged but don't fail the operation.
#[cfg(feature = "cdc_nats")]
pub fn op_cdc_publish(
    program: &Program,
    state: &mut ProgramState,
    insn: &Insn,
    _pager: &Arc<Pager>,
) -> Result<InsnFunctionStepResult> {
    load_insn!(CdcPublish { start_reg }, insn);

    // Check if a CDC sink is configured
    if let Some(sink) = program.connection.get_cdc_sink() {
        ...
        let change_time = match state.registers[*start_reg].get_value() {
            Value::Integer(i) => *i,
            _ => 0,
        };

        let change_type_int = match state.registers[*start_reg + 1].get_value() {
            Value::Integer(i) => *i,
            _ => 0,
        };
        let Some(change_type) = crate::cdc::CdcChangeType::from_i64(change_type_int) else {
            tracing::warn!("Invalid CDC change type: {}", change_type_int);
            state.pc += 1;
            return Ok(InsnFunctionStepResult::Step);
        };

        let table_name = match state.registers[*start_reg + 2].get_value() {
            Value::Text(t) => t.as_str().to_string(),
            _ => String::new(),
        };

        let id = state.registers[*start_reg + 3].get_value().clone();
        ...
        let updates = match state.registers[*start_reg + 6].get_value() {
            Value::Blob(b) => Some(b.clone()),
            _ => None,
        };

        let event = crate::cdc::CdcEvent {
            change_id: 0, // Not available at this point (auto-generated by NewRowid)
            change_time,
            change_type,
            table_name,
            id,
        ...
        // Fire-and-forget: ignore errors per requirement
        if let Err(e) = sink.publish(event) {
            tracing::debug!("CDC publish failed (ignored): {}", e);
        }
    }
